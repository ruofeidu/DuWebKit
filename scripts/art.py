"""Art defines a single art.

This class allows the user to format an art.
"""
import re
import math
import warnings
...
class Art:
  ...
  def __init__(self, row):
    ...
    self._data = {}
    b = self._data
    for i, label in enumerate(Art.get_header()):
  ...
  def get_bibtex(self):
    ...
    for label in Art.get_bib_items():
  ...
  def get_teaser_markdown(self):
    ...
    return DuMark.get_image('/%s/%s.jpg' % (Dir.arts, self.get_bib_id()),
